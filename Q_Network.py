...
# Parameter
# Input and Output size based on Env
...
Qpred = tf.matmul(X, W) # Return Q prediction
    ...
    for i in range(num_episodes):
        ...
        while not done:
            ...
            if done:
                ...
                # Obtain the Q, s1 values by feeding new new state through our network
                Qs1 = sess.run(Qpred, feed_dict={X: one_hot(s1)})
                # Update Q
                Qs[0, a] = reward + dis * np.max(Qs1)

            # Train our network using target Y and predicted Q values
